'''
Code adapted from scikit-plot. 
Fixed one defect in the original code: ColorBar drawn to ax.
'''
import itertools
...
def plot_confusion_matrix(y_true, y_pred, labels=None, true_labels=None,
                          ...
                          ax=None, cax = None, figsize=None, 
                          cmap='Blues', title_fontsize="large",
    ...
    if cax is not None:
        plt.colorbar(mappable=image, cax=cax) # use its own ax cax
    # else:
    #     plt.colorbar(mappable=image, ax=ax) # still space from ax
